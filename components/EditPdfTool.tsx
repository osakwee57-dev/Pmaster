...
const EditPdfTool: React.FC<PDFToolProps> = ({ onBack, initialData, draftId }) => {
  ...
  const handleConvertUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      try {
        ...
        setExtractedText("Failed to extract text. This document might be an image scan without a text layer. Try the Scan tool for OCR.");
  ...
  const performConversion = async (format: 'doc' | 'txt' | 'md' | 'json') => {
    ...
    setIsProcessing(true);
    setConversionStatus(`Building ${format.toUpperCase()} export...`);
    ...
    try {
      ...
      switch(format) {
        case 'doc':
          const htmlContent = `
            ...
            <body style="font-family: Arial, sans-serif; line-height: 1.6; padding: 40px;">
              <h1 style="color: #2563eb;">${convertFile.name}</h1>
              <p style="color: #64748b; font-size: 12px; margin-bottom: 30px;">Converted via PDF Master Toolset</p>
        ...
        case 'txt':
          ...
          break;
        case 'md':
          const md = `# ${convertFile.name}\n\nConverted via PDF Master Toolset\n\n${extractedText}`;
        ...
        case 'json':
          const data = {
            metadata: {
              source: convertFile.name,
              size: convertFile.size,
              pageCount: extractedPages.length,
              timestamp: new Date().toISOString()
            },
            pages: extractedPages.map((text, i) => ({ pageNumber: i + 1, content: text }))
          };
      ...
      downloadBlob(blob, `${convertFile.name.replace('.pdf', '')}.${ext}`);
      setConversionStatus("Success: File downloaded.");
    } catch (err) {
      setConversionStatus("Error: Failed to build export.");
    } finally {
      setIsProcessing(false);
    }
  };
      ...
      {activeTab === 'merge' ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
          <div className="bg-white p-8 rounded-[3rem] shadow-xl border border-slate-100 flex flex-col min-h-[500px]">
            <div className="flex items-center space-x-3 mb-8">
               ...
               <div>
                 ...
                 <p className="text-[10px] font-bold text-slate-400">Deep Text Extraction Engine</p>
               </div>
            </div>
            ...
            {!convertFile ? (
              ...
              <div className="flex-1 space-y-6 flex flex-col">
                <div className="bg-slate-50 p-6 rounded-3xl border border-slate-100 flex items-center">
                  ...
                  <button onClick={() => { setConvertFile(null); setExtractedText(""); }} className="p-3 text-slate-300 hover:text-red-500 transition-colors"><X className="w-5 h-5" /></button>
                </div>

                <div className="flex-1 bg-slate-900 rounded-[2rem] p-6 font-mono text-[11px] overflow-hidden flex flex-col">
                   <div className="flex items-center justify-between mb-3 border-b border-white/10 pb-2">
                      <span className="text-blue-400/80 uppercase font-black tracking-widest text-[9px]">Live Extraction Log</span>
                      <span className={`text-[8px] font-bold ${isProcessing ? 'text-yellow-400 animate-pulse' : 'text-emerald-400'}`}>
                        {isProcessing ? 'BUSY' : 'READY'}
                      </span>
                   </div>
                   <div className="flex-1 overflow-y-auto text-blue-300/60 leading-relaxed whitespace-pre-wrap">
                      {isProcessing ? (
                        <div className="flex items-center space-x-2">
                        ...
                        extractedText || "No text layers found in this document."
                      )}
                   </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3 pt-4">
                   <button 
                     ...
                     className="flex items-center justify-between p-5 bg-white border-2 border-slate-100 rounded-2xl hover:border-blue-500 hover:bg-blue-50 transition-all group disabled:opacity-50"
                   >
                   ...
                   <button 
                     ...
                     className="flex items-center justify-between p-5 bg-white border-2 border-slate-100 rounded-2xl hover:border-emerald-500 hover:bg-emerald-50 transition-all group disabled:opacity-50"
                   >
                     <div className="text-left">
          ...
          <div className="bg-slate-900 p-8 rounded-[3rem] shadow-2xl flex flex-col justify-center text-white space-y-6">
             ...
             <div className="space-y-2">
                <h3 className="text-2xl font-bold tracking-tight">Proper PDF Parsing</h3>
                <p className="text-slate-400 text-sm leading-relaxed">
                   We use a browser-side virtual machine to traverse the PDF object model, allowing us to reconstruct the text stream accurately.
                </p>
             </div>
             <ul className="space-y-4 pt-4">
                <li className="flex items-start space-x-3">
                   <Check className="w-5 h-5 text-emerald-400 shrink-0" />
                   <span className="text-xs text-slate-300">Word exports use HTML-Wrapper technology for layout consistency.</span>
                </li>
                <li className="flex items-start space-x-3">
                   <Check className="w-5 h-5 text-emerald-400 shrink-0" />
